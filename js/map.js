...
;!function (win, $, undefined) {
	var SVGMap = (function () {
		...
		SVGMap.prototype.options = {
			...
			mapWidth: 100+"%",
			mapHeight: 100+"%",
			stateColorList: ['d07d49', 'cad9ee', 'dbba8f', 'e4e4e4'],  //控制每块的颜色集合
		...
		SVGMap.prototype.scaleRaphael = function (container, width, height) {
			...
			var nestedWrapper;
			if (Raphael.type == "VML") {
				wrapper.innerHTML = "<rvml:group style='position : absolute; width: 400; height: 500; top: 0px; left: 0px' coordsize='1000,1000' class='rvml' id='vmlgroup_" + container + "'><\/rvml:group>";
				nestedWrapper = document.getElementById("vmlgroup_" + container);
			} else {
				wrapper.innerHTML = '<div class="svggroup" style="width:100%;height:100%;"></div>';
